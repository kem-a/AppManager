...
- **One-click installs from the web**: Registers the `x-scheme-handler/appmgr` handler, so an `appmgr://install?url=…&sha256=…` link on a store page (e.g. [AppHub](https://xlc-dev.github.io/apphub/)) downloads the AppImage - only over HTTPS, verified against the size and SHA-256 in the link - and hands it to the normal install flow.
