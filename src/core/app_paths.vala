namespace AppManager.Core {
    public class AppPaths {
        ...
        /**
         * Directory holding one <record-id>.args file per sandboxed app.
        ...
        /**
         * Path to a record's sandbox argument file. Does not create the file.
         */
        public static string sandbox_args_file(string record_id) {
            return Path.build_filename(sandbox_dir, "%s.args".printf(record_id));
        }

        /**
         * Path to the sas (simple-appimage-sandbox) launcher, or null if unavailable.
         * Same resolution order as zsync_path: explicit override, then PATH — which
         * covers both a native install and the bundled copy an AppImage prepends.
         */
        public static string? sas_path {
            owned get {
                var env_path = Environment.get_variable("APP_MANAGER_SAS_PATH");
                if (env_path != null && env_path.strip() != ""
                    && FileUtils.test(env_path.strip(), FileTest.IS_EXECUTABLE)) {
                    return env_path.strip();
                }
                var found = Environment.find_program_in_path("sas");
                if (found != null && found.strip() != "") {
                    return found;
                }
                return null;
        ...
        /**
         * Whether sandboxing can be offered at all: sas plus the bubblewrap binary
         * it drives. AppManager running inside a Flatpak has no usable bwrap, so
         * the feature is hidden there (see Installer.is_flatpak_sandbox).
         */
        public static bool sandbox_available {
            get {
                ...
                if (Environment.find_program_in_path("bwrap") == null) {
                    return false;
                }
                return sas_path != null;
            }
        }
